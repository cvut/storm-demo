 ...
 * counts of objects, and 2) to give a single "consumer" thread (e.g.
 * {@link PeriodicSlidingWindowCounter}) read access
 * to the counter. Whenever the consumer thread performs a read operation, this
 * class will advance the head slot of the
 ...
 * <code>windowLengthInSlots</code>
 * iterations, this sliding window counter will always return object counts that
 ...
 * in the past five minutes", implying that it can only account for the last two
 * of those five minutes because the
